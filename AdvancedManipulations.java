...
public class AdvancedManipulations {
    ...
    public static PixelPicture adjustContrast(
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                avg += p.getRed() + p.getGreen() + p.getBlue();
                size++;
            }
        }
        int newAvg = avg / size / 3;
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int newR = ((int) (Math.round((p.getRed() - newAvg) * multiplier)))
                        + newAvg;
                int newG = ((int) (Math.round((p.getGreen() - newAvg) * multiplier)))
                        + newAvg;
                int newB = ((int) (Math.round((p.getBlue() - newAvg) * multiplier)))
                        + newAvg;
    ...
    public static PixelPicture reducePalette(PixelPicture pic, int numColors) {
        ...
        Pixel[] array = m.getSortedPixels();
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                Pixel p = bmp[row][col];
                int min = p.distance(array[0]);
                bmp[row][col] = array[0];
                for (int i = 1; i < numColors; i++) {
                    if (p.distance(array[i]) < min) {
    ...
    public static PixelPicture blur(PixelPicture pic, int radius) {
        ...
        for (int col = 0; col < w; col++) {
            for (int row = 0; row < h; row++) {
                ...
                int newG = 0;
                int newB = 0;
                double div = (radius * 2 + 1) * (radius * 2 + 1);
                for (int c = col - radius; c <= col + radius; c++) {
                    for (int r = row - radius; r <= row + radius; r++) {
                        if (c < 0 || r < 0 || c > w - 1 || r > h - 1) {
                            div -= 1;
                        } else {
                            Pixel source = src[r][c];
                            newR += source.getRed();
                            newG += source.getGreen();
                            newB += source.getBlue();
                        }
                    }
                }
                int avgR = (int) Math.round(newR / div);
                int avgG = (int) Math.round(newG / div);
                int avgB = (int) Math.round(newB / div);
                tgt[row][col] = new Pixel(avgR, avgG, avgB);
            }
        }
