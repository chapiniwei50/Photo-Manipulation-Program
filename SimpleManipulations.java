...
public class SimpleManipulations {
    ...
    public static PixelPicture border(
        ...
        Pixel [][] src = pic.getBitmap();
        Pixel [][] b = new Pixel [h + 2 * borderWidth][w + 2 * borderWidth];


        for (int col = 0; col < w + 2 * borderWidth; col++) {
            for (int row = 0; row < h + 2 * borderWidth; row++) {
                if (col < borderWidth || col >= w + borderWidth || row < borderWidth ||
                        row >= h + borderWidth) {
                    b[row][col] = borderColor;
                } else {
                    b[row][col] = src[row -  borderWidth][col - borderWidth];
                }
    ...
    public static PixelPicture alphaBlend(
        ...
        Pixel[][] bmp = pic.getBitmap();
        Pixel[][] bmf = f.getBitmap();
        for (int col = 0; col < pw; col++) {
            for (int row = 0; row < ph; row++) {
                Pixel p = bmp [row][col];
                Pixel fx = bmf [row][col];
                int pr = p.getRed();
                int pg = p.getGreen();
                int pb = p.getBlue();
                int fr = fx.getRed();
                int fg = fx.getGreen();
                int fb = fx.getBlue();
                int newR = weightedAverage(alpha, pr,fr);
                int newG = weightedAverage(alpha, pg,fg);
                int newB = weightedAverage(alpha, pb,fb);
                bmp[row][col] = new Pixel(newR,newG,newB);
            }
        }
        return new PixelPicture(bmp);
